#include <obj_recognition/detect_sa.h>
#include <math.h>
...
bool ObjDetector::initialize()
{
    ...
    /*we dont have camera info from panorama*/
    std::string panorama_frame = nh.resolveName("/panorama_frame");
    
    camera_frame_id_ = panorama_frame;
    im_size_.height = M_PI; //fixed now to compile
    im_size_.width = 2*M_PI;//fixed now to compile
  
  
    /*load the calibration files of the SA cameras*/
    // load SA camera models
     // get SA camera model filenames
    std::string sa_left_model_file, sa_right_model_file;
    nh_local.getParam("sa_left_model", sa_left_model_file);
    nh_local.getParam("sa_right_model", sa_right_model_file);

//     ocam_model sa_left_model, sa_right_model;
//     if(get_ocam_model(&sa_left_model, sa_left_model_file.c_str()) == -1 ||
//        get_ocam_model(&sa_right_model, sa_right_model_file.c_str()) == -1)
//     {
//         ROS_FATAL("[AtlasPanorama] Loading SA model files failed!\n");
//         return false;
//     }
//     else 
//     {
//         ROS_INFO("[AtlasPanorama] loaded SA camera model from \"%s\" and \"%s\".\n", 
//                  sa_left_model_file.c_str(), sa_right_model_file.c_str());
//     }
...
void ObjDetector::detectCallback(const trooper_adaptive_perception_msgs::ObjectDetectionRequestConstPtr &cmd){
  ...
  /*  int queue_size = 2; 
    ...
    ROS_INFO("subscribed to %s", image_sub_.getTopic().c_str());
    std::string topicInfocam = nh.resolveName("/cam_info");    
    im_info_sub_.subscribe(nh, topicInfocam, queue_size);
    ROS_INFO("subscribed to %s", im_info_sub_.getTopic().c_str());
    std::string stereo_topic = nh.resolveName("/stereo_cloud");
    stereo_pcl_sub_.subscribe(nh, stereo_topic, queue_size);    
    ROS_INFO("subscribed to %s", stereo_pcl_sub_.getTopic().c_str());
        
    image_sub_.registerCallback(boost::bind(ObjDetector::increment, &image_received_));
    im_info_sub_.registerCallback(boost::bind(ObjDetector::increment, &im_info_received_));
    stereo_pcl_sub_.registerCallback(boost::bind(ObjDetector::increment, &stereo_pcl_received_));
    
    //exact sync is required as disparity image is calculated from left image and has to have the same timestamp
    check_synced_timer_ = nh.createTimer(ros::Duration(15.0), boost::bind(&ObjDetector::checkInputsSynchronized, this));
    
    exact_sync_.reset(new ExactSync(ExactPolicy(queue_size), image_sub_, im_info_sub_, stereo_pcl_sub_));
    exact_sync_->registerCallback(boost::bind(&ObjDetector::dataCbSync, this, _1, _2, _3));
  
  
  stereo_ref_system_ = pc_msg->header.frame_id;
  pcl::fromROSMsg(*pc_msg, stereoPC_);
  gotPCstereo_ = true;  */
...
//     pcl::fromROSMsg(cloud, *scene_world);
   ...
   /* Eigen::Vector3f tcloud;
    Eigen::Quaterniond C2RQ;
    C2RQ.x() = cloud_to_robot.getRotation().x();
    C2RQ.y() = cloud_to_robot.getRotation().y();
    C2RQ.z() = cloud_to_robot.getRotation().z();
    C2RQ.w() = cloud_to_robot.getRotation().w();
    tcloud(0) = cloud_to_robot.getOrigin()[0];
    tcloud(1) = cloud_to_robot.getOrigin()[1];
    tcloud(2) = cloud_to_robot.getOrigin()[2];
    
    Eigen::Matrix3f MQ = C2RQ.toRotationMatrix().cast<float>();
    cloud_to_robotE_.topLeftCorner(3,3) =  MQ;
    cloud_to_robotE_.col(3) << tcloud(0),tcloud(1),tcloud(2), 1.0 ;*/
   ...
   pcl_ros::transformPointCloud(*scene_world,*scene,cloud_to_robot_); 
    ...
    if (color_segmentation_){
      extractClustersColor(scene,&clusters);
    }  
    else{      
      pcl::PointCloud<PointT>::Ptr tmp(new pcl::PointCloud<PointT>);
      PointT point;
      for(int i=1;i<scene->points.size();i++){
        point.x = scene->points[i].x;
        point.y = scene->points[i].y;
        point.z = scene->points[i].z;   
        tmp->points.push_back(point);
      }
    //  extractClusters(scene, &clusters, vis);//tmp
    }
    ...
    //visualize clusters
     ...
     for(int i = 0; i < clusters.size(); ++i)
     {
         ...
         pcl::visualization::PointCloudColorHandlerRandom<PointTc> cluster_handler(clusters[i]);
    ...
    for(int j=0; j < clusters[i].indices.size();j++){
        ...
        point.x = rgbpc_sampled->points[index].x;
        point.y = rgbpc_sampled->points[index].y;
        point.z = rgbpc_sampled->points[index].z;       
...
void ObjDetector::extractClustersColor(const pcl::PointCloud< PointT >::ConstPtr& scene, std::vector< boost::shared_ptr< pcl::PointCloud< PointT > > >* clusters){
    ...
    if(manual_segmentation_){
       /* std::ofstream file("/home/lpuig/OUTPUT.txt");
        if(!file.is_open())
        {
            ROS_FATAL("NOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO!\n");
            exit(1);
        }*/
        //cv::Rect roi(roi_[0].x,roi_[0].y, (roi_[1].x - roi_[0].x) , (roi_[1].y - roi_[0].y)); 
        for(size_t i=0;i<rgbpc_sampled->points.size();i++)
        {
            ...
            //project the 3D point to the sphere
            double norm = sqrt(xyz.x * xyz.x + xyz.y * xyz.y + xyz.z * xyz.z);            
            cv::Point3d xyz_sphere(xyz.x/norm, xyz.y/norm,xyz.z/norm);
            //compute the corresponding theta and phi
            double phi = atan2(-xyz_sphere.y,-xyz_sphere.x);
            double theta = acos(xyz_sphere.z);
            //verify that the phi and theta computed are insisde the selected area
            im_point.x = (M_PI-phi)/(2*M_PI);
            im_point.y = theta/M_PI;
            
           // file <<  im_point.x << "," << im_point.y << ", " << norm << std::endl;
            if(norm < 28 && (im_point.x >= roi_[0].x) && (im_point.y >= roi_[0].y) && (im_point.x <= roi_[1].x) && (im_point.y <= roi_[1].y) )
            {
...
// //     extractor.filter(*cloud_segmented);
    ...
    const float Z_MIN = 0.07, Z_MAX = 2; //FIXME: coordinate system might be different
