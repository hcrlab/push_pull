  ...
  for (std::size_t i = 0; i < pcloud->points.size(); i++) {
    cv::Point3d point,
        xyz(pcloud->points[i].x, pcloud->points[i].y,
            pcloud->points[i].z);  // a Point3d of the point in the pointcloud
    ...
    if (u <= mask.rows && u >= 1 && v <= mask.cols && v >= 1)
      if (xyz.z < 1.0)
        if (mask.at<uchar>(u, v) > 0) subcluster->push_back(pcloud->points[i]);
  }
  return subcluster;
}
...
void MatchDescriptor::computeDescriptors(
  ...
  for (int icont = 0; icont < localcontours.size(); icont++) {
    cv::RotatedRect minRect = cv::minAreaRect(cv::Mat(localcontours[icont]));
...
bool MatchDescriptor::matchCallback(MatchCluster::Request &request,
  ...
  computeDescriptors(image, mask, keypointsMSERdef, &descriptors_MSER, histSize,
                     &submasks);

  std::cout << "\n\n\nBLOBS DETECTED  " << keypointsMSERdef.size() << std::endl;
  ...
  for (int i = 0; i < nobjects; i++) {
    ...
    for (int j = 0; j < matchesdef.size(); j++) {
      if (debug_) {
        ...
        cv::Mat mask3C;
        cv::cvtColor(submasks[matchesdef[j].queryIdx], mask3C, CV_GRAY2BGR);
      ...
      std::cout << request.match.descriptor_type << " descriptors computed "
           ...
           maxinliers))  // && (mindistMSER == matchesdef[j].distance))
      {
        ...
        // IT COULD BE THE SUBCLUSTER GIVEN BY THE SUBMASK OF THE MATCHED OBJECT
        // cluster <--  submasks[matchesdef[match]]
        /******************************************/
        pcl::PointCloud<pcl::PointXYZRGB>::Ptr subcl;
        subcl = subcluster(submasks[matchesdef[match].queryIdx], transf_pc);
    ...
    if (debug_) {
      ...
      cv::Mat mask3C;
      cv::cvtColor(submasks[matchesdef[match].queryIdx], mask3C, CV_GRAY2BGR);
      cv::addWeighted(image, 0.4, mask3C, 0.7, 0.0, img_matches);
