...
RobotDriver::RobotDriver() : node_handle_(), listener_() {
...
bool RobotDriver::DriveLinear(double dx, double dy, double distance) {
  ...
  listener_.waitForTransform("base_footprint", "odom_combined", ros::Time(0),
                             ros::Duration(1.0));

  tf::StampedTransform start_transform;
  tf::StampedTransform current_transform;

  listener_.lookupTransform("base_footprint", "odom_combined", ros::Time(0),
                            start_transform);
...
bool RobotDriver::DriveAngular(double dt, double radians) {
  ...
  if (std::abs(dt) >= 2 * M_PI) {
    ROS_ERROR(
        "DriveAngular not tested for abs(dt) > 2pi radians,"
        "aborting");
    return false;
  }
...
bool RobotDriver::DriveToPose(geometry_msgs::PoseStamped pose,
  ...
  tf::Matrix3x3 m(
      tf::Quaternion(newPose.pose.orientation.x, newPose.pose.orientation.y,
                     newPose.pose.orientation.z, newPose.pose.orientation.w));
  ...
  double distance =
      sqrt(pow(newPose.pose.position.x, 2) + pow(newPose.pose.position.y, 2));
